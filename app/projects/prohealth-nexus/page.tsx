...
function DashboardShowcase({ title, lightSrc, darkSrc, description }: DashboardProps) {
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-4">
        <button onClick={() => setMode("light")} className={`px-4 py-1.5 rounded-full text-sm font-medium transition ${mode === "light" ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>Light Mode</button>
        <button onClick={() => setMode("dark")} className={`px-4 py-1.5 rounded-full text-sm font-medium transition ${mode === "dark" ? "bg-gray-900 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>Dark Mode</button>
      </div>
...
export default function ProHealthCaseStudy() {
  ...
  const impacts = [
    { icon: faArrowTrendUp, stat: "40%", label: "Reduction in communication delays", description: "Through real-time notifications and instant appointment updates between patients and doctors." },
    { icon: faUserDoctor, stat: "3x", label: "Faster patient record access", description: "Doctors can pull up complete medical histories instantly instead of searching through fragmented systems." },
    { icon: faHospitalUser, stat: "100%", label: "Role-based access control", description: "Patients, doctors and admins each see only what they need — keeping data secure and workflows clean." },
    { icon: faChartLine, stat: "↑", label: "Higher patient engagement", description: "A clean, intuitive UI across all devices led to higher satisfaction and more consistent platform usage." },
  ];
    ...
    <main className="min-h-screen bg-white">
      ...
      <div className="max-w-3xl mx-auto px-6 pt-32 pb-24">
        ...
        <div className="flex flex-col gap-12">
          ...
          <DashboardShowcase title="Patient Dashboard" lightSrc="/images/prohealth/patient.png" darkSrc="/images/prohealth/patient-dark.png" description="Patients can book appointments, track their medical history, download records as PDF and receive automated email reminders — reducing missed appointments and keeping them informed at every step." />
          <DashboardShowcase title="Doctor Dashboard" lightSrc="/images/prohealth/doctor.png" darkSrc="/images/prohealth/doctor-dark.png" description="Doctors get a real-time view of their schedule, instant access to patient histories and the ability to update records on the go — enabling faster, better-informed clinical decisions." />
          <DashboardShowcase title="Admin Dashboard" lightSrc="/images/prohealth/admin.png" darkSrc="/images/prohealth/admin-dark.png" description="Admins manage the entire platform from one place — overseeing users, monitoring appointments and ensuring smooth operations without needing to switch between multiple tools." />
